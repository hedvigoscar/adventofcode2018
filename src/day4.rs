...
pub fn parse_day4(input: &str) -> Vec<Record> {
    let mut input = input
        ...
        .map(|s| {
            ...
            let date = NaiveDateTime::parse_from_str(date, "[%Y-%m-%d %H:%M]").expect("Failed to parse date");

            let record_type = if rest.contains("begins shift") {
                let id = rest.split_whitespace().nth(1).expect("Problem unwrapping nth(1) element");
...
pub fn solve_day4_part1(input: &[Record]) -> u32 {
    ...
    for i in input {
        match &i.record_type {
            ...
            RecordType::WakesUp => {
                ...
                assert!(minutes_asleep > 0);
                let minutes_currently_slept = minutes_slept_by_guard.get(&current_guard_id).unwrap_or(&0);
                minutes_slept_by_guard.insert(current_guard_id.clone(), minutes_currently_slept + minutes_asleep as u64);
            }
        }
    }
    ...
    let max_slept_guard_id = max_slept_guard_id.parse::<u32>().expect("Error parsing guard id");
